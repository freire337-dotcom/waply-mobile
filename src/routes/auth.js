...
router.post('/login', async (req, res) => {
  try {
    ...
    const tenant = await db.prepare('SELECT id FROM tenants WHERE slug = ? AND active = 1').get(tenant_slug);
...
router.post('/fcm-token', auth, async (req, res) => {
  try {
    ...
    await db.prepare('UPDATE agents SET fcm_token = ? WHERE id = ?').run(fcm_token, req.agent.id);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: 'Error interno' });
  }

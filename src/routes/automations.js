...
router.get('/:id', auth, async (req, res) => {
  try {
    const auto = await db.prepare('SELECT * FROM automations WHERE id = ? AND tenant_id = ?')
...
router.patch('/:id', auth, async (req, res) => {
  try {
    ...
    const auto = await db.prepare('SELECT id FROM automations WHERE id = ? AND tenant_id = ?')
      .get(req.params.id, req.agent.tenant_id);
    if (!auto) return res.status(404).json({ error: 'No encontrada' });
    ...
    await db.prepare(`UPDATE automations SET ${fields.join(', ')} WHERE id = ?`).run(...values);
    res.json({ ok: true });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Error interno' });
...
router.delete('/:id', auth, async (req, res) => {
  try {
    if (req.agent.role !== 'admin') return res.status(403).json({ error: 'Solo admins' });
    await db.prepare('UPDATE automations SET active = 0 WHERE id = ? AND tenant_id = ?')
